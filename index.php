<?php
/*
Plugin Name: WP Hooks Locator
Plugin URI: https://github.com/kipmyk/WP-Hooks-Locator
...
Text Domain: wp-hooks-locator
Domain Path: /languages
*/

defined( 'ABSPATH' ) or die( 'No Trespassing!' ); // Security

class mk_Hooks_Locator {
...
public function set_active_status() {
	...
	function admin_bar_menu( $wp_admin_bar ) {
		...
		if ( 'show-action-hooks' == $this->status ) {
			...
			$title 	= __( 'Stop Showing Action Hooks' , 'wp-hooks-locator' );
			$href 	= add_query_arg( 'cxssh-hooks', 'off', $url );
			$css 	= 'cxssh-hooks-on cxssh-hooks-normal';
		}
		else {
			
			$title 	= __( 'Show Action Hooks' , 'wp-hooks-locator' );
			$href 	= add_query_arg( 'cxssh-hooks', 'show-action-hooks', $url );
			$css 	= '';
		}
		
		$wp_admin_bar->add_menu( array(
			'title'		=> '<span class="ab-icon"></span><span class="ab-label">' . __( 'WP Hooks Locator' , 'wp-hooks-locator' ) . '</span>',
			'id'		=> 'cxssh-main-menu',
			'parent'	=> false,
			'href'		=> $href,
		) );
		...
		if ( $this->status=="show-filter-hooks" ) {
			...
			$title	= __( 'Stop Showing Action & Filter Hooks' , 'wp-hooks-locator' );
			$href 	= add_query_arg( 'cxssh-hooks', 'off', $url );
			$css 	= 'cxssh-hooks-on cxssh-hooks-sidebar';
		}
		else {
			
			$title	= __( 'Show Action & Filter Hooks' , 'wp-hooks-locator' );
	...
	function notification_switch() {
		...
		<a class="cxssh-notification-switch" href="<?php echo esc_url( $url ); ?>">
			...
			<?php echo _e( 'Stop Showing Hooks' , 'wp-hooks-locator' ); ?>
		</a>
		<?php
	...
	public function load_translation() {
		load_plugin_textdomain( 'wp-hooks-locator', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
					...
					<ul class="cxssh-hook-dropdown">
						...
						foreach ( $nested_hooks as $nested_key => $nested_value ) :
							...
							<li class="cxssh-priority">
								<span class="cxssh-priority-label"><strong><?php echo 'Priority:'; /* _e('Priority', 'wp-hooks-locator') */ ?></strong> <?php echo $nested_key ?></span>
							</li>
							<?php
